...
def restrict_to_classes(loader, i_classes, remap_labels):
    ...
    for key in keys:
        if hasattr(loader.dataset, key):
            found_key = key
            if key != 'tensors':
                labels = getattr(loader.dataset, key)
            else:
                labels = loader.dataset.tensors[1].argmax(dim=1).detach().cpu().numpy()
            indices = [np.flatnonzero(np.array(labels) == i_class) for i_class in i_classes]
            indices = np.sort(np.concatenate(indices))

    if found_key is None:
        assert hasattr(loader.dataset, 'tensors')
    ...
    if found_key == 'tensor':
        new_labels = F.one_hot(
            th.tensor(labels), num_classes=loader.dataset.tensors[1].shape[1]
        ).type_as(loader.dataset.tensors[1])[indices].clone()
        subset = TensorDataset(loader.dataset.tensors[0][indices].clone(),
                               new_labels)
    else:
        loader.dataset.__dict__[found_key] = labels
        subset = th.utils.data.Subset(loader.dataset, indices)
