...
def split_bcic_iv_2a_train_valid_test(windows_dataset, split_valid_off_train, all_subjects_in_each_fold):
    train_whole_set = windows_dataset.split('session')['session_T']
    eval_whole_set = windows_dataset.split('session')['session_E']
    if split_valid_off_train:
        ...
        if all_subjects_in_each_fold:
            ...
            test_set = BaseConcatDataset([run_splitted[f'run_{i}'] for i in range(4, 6)])
        else:
...
def load_train_valid_test_bcic_iv_2a(subject_id, class_names, split_valid_off_train, all_subjects_in_each_fold):
    preproced_set = load_and_preproc_bcic_iv_2a(subject_id)
    windows_dataset = create_window_dataset(preproced_set, class_names)
